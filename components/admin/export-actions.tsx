...
const labels: Record<string, string> = { id: "شناسه", status: "وضعیت", fullName: "نام کامل", name: "نام", email: "ایمیل", phone: "موبایل", nationalCode: "کد ملی", course: "دوره", title: "عنوان", orderNumber: "شماره سفارش", amountTomans: "مبلغ (تومان)", amountRials: "مبلغ (ریال)", method: "روش پرداخت", createdAt: "تاریخ ایجاد", updatedAt: "تاریخ بروزرسانی", paidAt: "تاریخ پرداخت", province: "استان", city: "شهر", address: "نشانی", postalCode: "کدپستی", educationLevel: "مقطع تحصیلی", educationField: "رشته تحصیلی", reason: "دلیل انتخاب", workHistory: "سوابق کاری", artHistory: "سوابق هنری", receiptUrl: "رسید پرداخت", discountLabel: "گروه تخفیف", discountPercent: "درصد تخفیف", finalAmountTomans: "مبلغ نهایی", manualReference: "شماره پیگیری" };
const values: Record<string, string> = { pending: "در انتظار پرداخت", awaiting_receipt: "در انتظار رسید", under_review: "نیازمند بررسی", paid: "موفق", rejected: "رد شده", expired: "منقضی", approved: "تأیید شده", pending_payment: "در انتظار پرداخت", card_to_card: "کارت به کارت", bale_wallet: "کیف پول بله", manual: "پرداخت دستی", true: "بله", false: "خیر" };

function flatten(value: unknown, prefix = "", result: Record<string, string | number | boolean> = {}) { if (value === null || value === undefined) { result[prefix] = ""; return result; } if (typeof value !== "object") { const raw = typeof value === "string" && /At$/.test(prefix) ? new Date(value).toLocaleString("fa-IR") : value; result[prefix] = typeof raw === "string" || typeof raw === "boolean" ? values[String(raw)] || raw : raw as number; return result; } Object.entries(value as Record<string, unknown>).forEach(([key, child]) => flatten(child, prefix ? `${prefix}.${key}` : key, result)); return result; }
