...
export async function issueEmailVerificationCode(email: string, name: string, purpose = "register") {
  const normalized = email.trim().toLowerCase();
  const latest = await prisma.verificationCode.findFirst({ where: { destination: normalized, channel: "email", purpose }, orderBy: { createdAt: "desc" } });
  if (latest && Date.now() - latest.createdAt.getTime() < 60_000) throw new Error("RATE_LIMIT");
  const code = crypto.randomInt(100000, 1000000).toString();
  await prisma.verificationCode.deleteMany({ where: { destination: normalized, channel: "email", purpose } });
  const record = await prisma.verificationCode.create({ data: { destination: normalized, channel: "email", codeHash: hashCode(normalized, code), purpose, expiresAt: new Date(Date.now() + 10 * 60_000) } });
  try {
...
export async function verifyEmailCode(email: string, code: string, purpose = "register") {
  const normalized = email.trim().toLowerCase();
  const record = await prisma.verificationCode.findFirst({ where: { destination: normalized, channel: "email", purpose }, orderBy: { createdAt: "desc" } });
