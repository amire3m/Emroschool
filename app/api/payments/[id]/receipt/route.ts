...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      if (attempt) await tx.paymentAttempt.update({ where: { id: attempt.id }, data: { receiptUrl, status: "under_review", submittedAt: new Date(), rejectionReason: null } });
      return tx.paymentOrder.update({ where: { id: order.id }, data: { receiptUrl, status: "under_review", receiptSubmittedAt: new Date(), rejectionReason: null } });
    });
