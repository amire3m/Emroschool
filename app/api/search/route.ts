...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const results = [
      ...
      ...news.map((item) => ({ id: item.id, type: "news", title: item.title, description: item.excerpt, image: item.coverImage, meta: item.category, date: item.publishedAt, url: `/news/${item.slug}` })),
    ];
