  ...
  try {
    const [courses, events, instructors, alumni, news] = await Promise.all([
      ...
      prisma.event.findMany({
        ...
        select: { id: true, title: true, slug: true, description: true, imageUrl: true, location: true, startDate: true },
        take: 12,
      }),
    ...
    const results = [
      ...
      ...events.map((item) => ({ id: item.id, type: "event", title: item.title, description: item.description, image: item.imageUrl, meta: item.location, date: item.startDate, url: `/events/${item.slug}` })),
