...
export default function AdminNewsPage() {
  ...
  return <div className="space-y-5">
    ...
    {modalOpen && <div className="modal-overlay" onClick={() => !saving && setModalOpen(false)}><form onSubmit={save} className="modal-content max-w-4xl" onClick={(event) => event.stopPropagation()}><div className="flex items-center justify-between mb-5"><div><h2 className="text-xl font-black text-primary">{editing ? "ویرایش خبر" : "روایت تازه"}</h2><p className="text-xs text-outline mt-1">متن را با یک خط خالی بین پاراگراف‌ها بنویسید.</p></div><button type="button" onClick={() => setModalOpen(false)} className="p-2 text-outline"><X size={20} /></button></div><div className="grid md:grid-cols-2 gap-4">
      ...
      <label className="text-sm font-bold text-primary">اسلاگ انگلیسی<input required dir="ltr" value={form.slug} onChange={(event) => setForm((current) => ({ ...current, slug: event.target.value.toLowerCase().replace(/[^a-z0-9-]/g, "-").replace(/-+/g, "-") }))} className="mt-1 w-full px-4 py-3 rounded-xl border border-surface-variant font-normal outline-none focus:ring-2 focus:ring-secondary-fixed" /></label>
      ...
      <label className="text-sm font-bold text-primary">نام نویسنده<input value={form.authorName} onChange={(event) => setForm((current) => ({ ...current, authorName: event.target.value }))} placeholder="تحریریه مدرسه" className="mt-1 w-full px-4 py-3 rounded-xl border border-surface-variant font-normal" /></label><label className="text-sm font-bold text-primary">برچسب‌ها<input value={form.tags} onChange={(event) => setForm((current) => ({ ...current, tags: event.target.value }))} placeholder="هنر، رسانه، آموزش" className="mt-1 w-full px-4 py-3 rounded-xl border border-surface-variant font-normal" /></label>
