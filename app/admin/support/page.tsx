...
export default function SupportPage() {
  ...
  useEffect(() => { load(); }, []);
  const review = async (id: string, status: "approved" | "rejected") => { setReviewing(id); try { const response = await fetch(`/api/admin/users/${id}/profile-review`, { method: "POST", headers: { ...auth(), "Content-Type": "application/json" }, body: JSON.stringify({ status }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); toast.success(status === "approved" ? "پروفایل تایید شد" : "درخواست رد شد"); load(); } catch (error) { toast.error(error instanceof Error ? error.message : "ثبت بررسی ناموفق بود"); } finally { setReviewing(null); } };
