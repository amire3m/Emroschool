        ...
        <div className="modal-overlay" onClick={() => !saving && setEditUser(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              {editTab === "courses" && <div className="space-y-5"><section className="rounded-2xl border border-secondary-fixed/60 bg-[#fff8e9] p-4"><h4 className="text-sm font-black text-primary">افزودن دستی دوره</h4><p className="mt-1 text-xs leading-6 text-outline">این عمل کاربر را بدون ایجاد پرداخت یا درخواست ثبت‌نام در دوره انتخاب‌شده ثبت می‌کند.</p><div className="mt-3 flex flex-col gap-2 sm:flex-row"><select value={manualCourseId} onChange={(event) => setManualCourseId(event.target.value)} className="min-w-0 flex-1 rounded-xl border border-surface-variant bg-white px-3 py-2.5 text-sm"><option value="">انتخاب دوره</option>{courses.filter((course) => !editUser.enrollments?.some((enrollment) => enrollment.course.id === course.id)).map((course) => <option key={course.id} value={course.id}>{course.title}{course.scheduleStatus === "completed" ? " (برگزارشده)" : ""}</option>)}</select><button type="button" disabled={saving || !manualCourseId} onClick={addManualEnrollment} className="rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">افزودن دوره</button></div></section><section><h4 className="text-sm font-black text-primary">دوره‌های ثبت‌شده</h4><div className="mt-2 space-y-2">{editUser.enrollments?.map((enrollment) => <div key={enrollment.id} className="rounded-xl border border-surface-variant bg-surface-low p-3"><p className="font-bold text-primary">{enrollment.course.title}</p><p className="mt-1 text-xs text-outline">ثبت در دوره: {new Date(enrollment.createdAt).toLocaleDateString("fa-IR")} · پیشرفت: {enrollment.progress.toLocaleString("fa-IR")}٪ {enrollment.completed ? "· تکمیل‌شده" : ""}</p></div>)}{!editUser.enrollments?.length && <p className="rounded-xl bg-surface-low p-3 text-sm text-outline">کاربر هنوز در دوره‌ای ثبت نشده است.</p>}</div></section><section><h4 className="text-sm font-black text-primary">درخواست‌های ثبت‌نام و کد تخفیف</h4><div className="mt-2 space-y-2">{editUser.courseApplications?.map((application) => <div key={application.id} className="rounded-xl border border-surface-variant p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-bold text-primary">{application.course.title}</p><span className={`rounded-full px-2 py-1 text-[11px] font-bold ${application.status === "approved" ? "bg-green-50 text-green-700" : application.status === "rejected" ? "bg-error-container text-error" : "bg-yellow-50 text-yellow-700"}`}>{application.status === "approved" ? "تایید شده" : application.status === "rejected" ? "رد شده" : "در انتظار بررسی"}</span></div><p className="mt-1 text-xs text-outline">{new Date(application.createdAt).toLocaleDateString("fa-IR")}</p>{application.discountCode ? <p className="mt-2 rounded-lg bg-secondary-fixed/40 px-2 py-1.5 text-xs font-bold text-primary">کد تخفیف: {application.discountCode}{application.discountLabel ? ` · ${application.discountLabel}` : ""} · {application.discountPercent.toLocaleString("fa-IR")}٪ · مبلغ نهایی: {application.finalAmountTomans.toLocaleString("fa-IR")} تومان</p> : <p className="mt-2 text-xs text-outline">بدون کد تخفیف</p>}</div>)}{!editUser.courseApplications?.length && <p className="rounded-xl bg-surface-low p-3 text-sm text-outline">درخواست ثبت‌نامی وجود ندارد.</p>}</div></section></div>}
              {editTab === "access" && <>
              ...
              <div><label className="block text-sm font-medium text-primary mb-1">رمز عبور جدید</label><input type="password" name="newPassword" autoComplete="new-password" value={String(editForm.password)} onChange={(e) => setEditForm((current) => ({ ...current, password: e.target.value }))} minLength={6} placeholder="برای حفظ رمز فعلی خالی بگذارید" className="w-full px-3 py-2.5 rounded-xl border border-surface-variant text-sm focus:outline-none focus:ring-2 focus:ring-secondary-fixed" /><p className="text-xs text-outline mt-1">حداقل ۶ کاراکتر؛ مدیر می‌تواند برای کاربر رمز جدید تعیین کند.</p></div></>}
