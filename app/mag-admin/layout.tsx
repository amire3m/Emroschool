...
export default function MagazineAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const [magazineFont, setMagazineFont] = useState("foran");

  useEffect(() => {
    fetch("/api/magazine-settings").then((response) => response.json()).then((data) => { if (data.font) setMagazineFont(data.font); }).catch(() => {});
  ...
  return <div className="min-h-screen bg-[#f8f5ee] flex" dir="rtl" style={{ fontFamily: magazineFont === "kay" ? "Kay, sans-serif" : "Foran, sans-serif" }}>
