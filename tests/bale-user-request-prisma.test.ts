...
function bounded<T>(promise: Promise<T>, label: string, milliseconds = 10_000) {
  let timer!: NodeJS.Timeout;
  return Promise.race([
    promise,
    new Promise<never>((_, reject) => { timer = setTimeout(() => reject(new Error(`${label} timed out after ${milliseconds}ms`)), milliseconds); }),
...
test("real SQLite CAS commits one profile and receipt revision under contention", async () => {
  ...
  try {
    ...
    const profiles = await bounded(Promise.all([
      updateProfile(profileRequest(), {}, { db, authenticate: () => ({ id: user.id }), now: () => new Date("2026-08-12T12:00:00.000Z") } as never),
      updateProfile(profileRequest(), {}, { db, authenticate: () => ({ id: user.id }), now: () => new Date("2026-08-12T12:00:00.000Z") } as never),
    ]), "concurrent profile routes");
    assert.deepEqual(profiles.map((response) => response.status).sort(), [200, 409]);
    assert.equal((await db.user.findUniqueOrThrow({ where: { id: user.id } })).profileReviewRevision, 1);
    assert.equal(await db.baleGroupEvent.count({ where: { type: "profile_review", eventKey: { startsWith: `profile-review:${user.id}:` } } }), 1);
    ...
    const receipts = await bounded(Promise.all([
      submitReceipt(receiptRequest(), { params: { id: order.id } }, overrides),
      submitReceipt(receiptRequest(), { params: { id: order.id } }, overrides),
    ]), "concurrent receipt routes");
    assert.deepEqual(receipts.map((response) => response.status).sort(), [200, 409]);
    assert.equal((await db.paymentOrder.findUniqueOrThrow({ where: { id: order.id } })).receiptSubmissionRevision, 1);
    assert.equal(await db.baleGroupEvent.count({ where: { type: "payment_receipt", eventKey: { startsWith: `payment-receipt:${order.id}:` } } }), 1);
